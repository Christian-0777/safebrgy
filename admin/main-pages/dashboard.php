...
$requestTypeColumn = in_array('request_type', $requestsColumns, true)
    ? 'request_type'
    : (in_array('document_type', $requestsColumns, true) ? 'document_type' : null);

$createdAtColumn = in_array('created_at', $requestsColumns, true)
    ? 'created_at'
    : (in_array('submitted_at', $requestsColumns, true) ? 'submitted_at' : null);
...
if (count($recentActivities) < 10) {
    $activityJoinClause = $hasUserIdColumn
        ? 'LEFT JOIN users u ON r.user_id = u.id'
        : (in_array('resident_email', $requestsColumns, true)
            ? 'LEFT JOIN users u ON u.email = r.resident_email'
            : 'LEFT JOIN users u ON 1 = 0');

    $activityOrderColumn = $createdAtColumn ?: 'id';
    $requestTypeExpression = $requestTypeColumn ? "COALESCE(r.$requestTypeColumn, 'Document Request')" : "'Document Request'";
    ...
    $additionalActivities = $pdo->query(
        "SELECT 
            ...
            u.id as actor_id,
            u.username as actor,
            CONCAT('submitted a new request: ', $requestTypeExpression) as action,
            NULL as meta,
            r.$activityOrderColumn as created_at,
            'request' as activity_type
        FROM requests r
        $activityJoinClause
        ORDER BY r.$activityOrderColumn DESC
        LIMIT 5"
    )->fetchAll();
...
if ($requestTypeColumn) {
    $pendingDocuments['by_type'] = $pdo->query(
        "SELECT $requestTypeColumn as request_type, COUNT(*) as count 
        FROM requests 
        WHERE status = 'Pending' 
        GROUP BY $requestTypeColumn"
