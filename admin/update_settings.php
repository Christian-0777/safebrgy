...
try {
    ...
    if ($section === 'account') {
        ...
        if (!empty($_FILES['profileImage']['tmp_name'])) {
            if ($_FILES['profileImage']['size'] > 2 * 1024 * 1024 || !in_array($_FILES['profileImage']['type'], ['image/jpeg', 'image/png', 'image/webp'], true)) {
                throw new RuntimeException('Profile photo must be a JPG, PNG, or WEBP under 2 MB.');
            }
        ...
        $sql = 'UPDATE users SET username = :username, email = :email, phone = :phone' . ($profilePath ? ', profile_image = :profile_image' : '') . ' WHERE id = :id AND role = "admin"';
    ...
    if ($section === 'barangay') {
        ...
        $params = [trim($_POST['barangayName'] ?? ''), trim($_POST['barangayAddress'] ?? ''), trim($_POST['barangayContact'] ?? ''), trim($_POST['officialEmail'] ?? ''), trim($_POST['websiteUrl'] ?? ''), trim($_POST['systemDescription'] ?? ''), $adminId];
