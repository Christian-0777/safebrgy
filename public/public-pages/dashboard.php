...
if ($userId) {
    ...
    $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM requests WHERE user_id = ? AND status IN ("Approved", "Ready to Receive")');
    $stmt->execute([$userId]);
    ...
    // Count unread notifications (we'll check latest requests for now)
    $stmt = $pdo->prepare('SELECT COUNT(*) as count FROM requests WHERE user_id = ? AND status != "Received" AND updated_at > DATE_SUB(NOW(), INTERVAL 7 DAY)');
    $stmt->execute([$userId]);
            ...
            <ul class="summary-list">
              <?php foreach ($recentUpdates as $update): ?>
                <li class="summary-item">
                  <span class="update-type"><?php echo htmlspecialchars($update['request_type']); ?></span>
