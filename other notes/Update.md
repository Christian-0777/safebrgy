...
#### Overview
Fixed the resident profile photo workflow so a newly selected profile picture is saved when the resident clicks **Save Personal Information** and remains visible after refreshing or revisiting the profile page.

#### Highlights
- Connected the profile photo input to the personal-information form using `multipart/form-data`.
- Added the profile photo field name required for file submission.
- Updated the personal-information handler to validate and save uploaded profile photos.
- Added MIME type, image integrity, and 5 MB file-size validation.
- Saves the updated profile path to both `users.profile_image` and `residents.profile_image_path`.
- Uses the latest `users.profile_image` value when rendering the resident profile and account pages.
- Preserved the existing cover-photo upload workflow and synchronized `users.cover_photo` with `residents.cover_photo_path`.
- Added transactional database updates and cleanup of failed uploads.
- Kept profile and cover media paths compatible with the application root and clean URL routing.

#### Files Updated
- `public/public-pages/account.php`
- `public/public-pages/profile.php`
- `assets/js/public/account.js`
- `api/account/update_personal.php`
- `api/account/update_profile.php`
- `api/account/update_cover.php`

#### Validation and Database Checks
- Confirmed the `users.profile_image` and `users.cover_photo` columns are available.
- Confirmed resident media path columns are available in the `residents` table.
- Passed PHP syntax checks and JavaScript syntax validation.
- Verified that profile media updates persist after page reload.

### Resident Registration Verification Fixes

#### Overview
Fixed resident registration failures where a valid email verification code was reported as invalid or expired, and where submitting the code returned a generic validation error that redirected the applicant to an earlier registration step.

#### Highlights
- Standardized registration OTP creation, validation, and consumption on UTC timestamps to prevent PHP/MySQL timezone differences from expiring valid codes.
- Added server-side logging for registration OTP lookup failures while keeping sensitive database details out of production responses.
- Improved registration error handling so the exact rejected field and validation reason are displayed instead of only showing a generic 422 message.
- Added specific upload error messages for missing, incomplete, oversized, and server-limit-exceeded registration photos.
- Increased the registration request limit to support three required images up to 5 MB each, with a 20 MB total POST limit and up to 10 uploaded files.
...
#### Validation
- Passed PHP syntax checks for all registration OTP and account creation handlers.
- Passed JavaScript syntax validation for the registration wizard.
- Confirmed the OTP expiry and consumption queries use `UTC_TIMESTAMP()` consistently.
- Confirmed registration validation responses expose field-level errors to the browser.

## What's New in V3.3 (Admin Request Rejection Workflow)

### Admin Request Page Updates

#### Overview
Improved the admin request status workflow with a dedicated rejection confirmation modal and clearer reference-number terminology. Rejection notifications are now sent only after the administrator confirms the rejection.

#### Highlights
- Renamed the admin request table identifier from **Request #** to **Reference No.** for consistency across the request page.
- Added a dedicated **Reject Request** modal when the administrator selects **Rejected** and clicks **Update Status**.
- Displays the resident name, document type, and reference number in the rejection modal.
- Added predefined rejection reasons:
  - Incomplete requirements
  - Invalid or incorrect information
  - Invalid or expired identification
  - Information does not match barangay records
  - Duplicate request
  - Applicant is not eligible
  - Document requirements not met
  - Request submitted under the wrong document type
  - Supporting document is unclear or unreadable
  - Request cannot be verified
  - Other
- Added a required custom reason field when **Other** is selected.
- Added an optional **Additional Details** field for instructions or supporting information.
- Added a warning that the resident will be notified by email and/or SMS.
- Added server-side validation to require a rejection reason.
- Prevented rejected requests from being updated or notified before final confirmation.
- Preserved the existing direct update flow for non-rejection statuses.

#### Notification Updates
- Rejection email and SMS notifications are sent only after clicking **Reject Request**.
- The selected rejection reason and optional additional details are included in the resident notification.
- Notification logs include the rejection reason for audit tracking.
...
#### Validation and Database Checks
- Confirmed the `requests` table exists and contains the `reference_no` and `status` columns.
- Confirmed the configured MySQL connection is working.
- Passed PHP syntax checks and JavaScript syntax validation.
- Fixed stray request markup that caused undefined `$req` warnings when opening the admin request page.

## What's New in V3.2 (Hostinger Deployment and Landing Page Routing)

### Production Deployment Updates

#### Overview
Updated SafeBrgy for deployment on Hostinger with separate resident and administrator landing page URLs. The application now selects the correct landing experience based on the hostname.

#### Landing Page URLs
- **Resident portal**: `https://safebrgy.com/landing`
- **Admin portal**: `https://admin.safebrgy.com/landing`

#### Deployment Requirements
- Upload the project files to the Hostinger domain's `public_html` directory.
- Configure `safebrgy.com` and `admin.safebrgy.com` to use the same application directory.
- Keep `.htaccess` enabled so clean URL routing works correctly.
- Configure the production database credentials in the environment configuration before going live.
- Ensure PHP, MySQL, and the required upload-directory write permissions are enabled in Hostinger.

#### Routing and Security Changes
- Added hostname-based routing for the resident and admin `/landing` paths.
- Moved the admin landing page to `admin/index.php`.
- Added an admin-subdomain `robots.txt` policy that blocks indexing of all admin URLs.
- Added `X-Robots-Tag: noindex, nofollow, noarchive` headers for admin responses.
- Updated landing-page links so login, registration, legal pages, and public support links work from the production domains.

## What's New in V3.1 (Remember Me Authentication)

### Persistent Login for Resident and Admin Users

#### Overview
Added a secure Remember Me feature for both resident and administrator login flows. Users who select Remember Me can return to SafeBrgy after closing their browser without entering their credentials again during the token lifetime.

#### Highlights
- Added random selector and validator token generation for persistent logins
- Stores only a SHA-256 hash of the validator token in MySQL
- Stores the raw token only in a secure, HttpOnly, SameSite cookie
- Automatically restores the correct resident or admin PHP session on future visits
- Automatically redirects remembered residents from `/safebrgy` to the resident dashboard
- Automatically redirects remembered admins from `/safebrgy/admin` to the admin dashboard
- Landing pages remain available after logout because logout revokes the remember-me cookie
- Shows a cookie consent modal on the resident and admin landing pages for visitors without consent
- Stores consent only after Allow; Deny stores nothing, so the prompt appears again on a later visit
- Regenerates the PHP session ID when a remembered login is restored
- Rotates and removes the persistent token after successful restoration
- Validates account verification status before restoring a remembered session
- Revokes the remembered token and clears the cookie when the user logs out
- Uses a 30-day token expiration period

#### Database and Integration
- Added the `remember_tokens` table to the authoritative schema
- Added runtime migration support for existing installations
- Shared the remember-me implementation between resident and admin authentication
- Added automatic restoration from the resident and admin landing pages and login pages
- Added `/safebrgy/admin` routing to the admin landing page and clean dashboard redirects

#### Files Updated
- `includes/shared/remember_me.php`
- `public/login.php`
- `public/logout.php`
- `admin/login.php`
...
- `.htaccess`
- `assets/css/shared/cookie-consent.css`
- `assets/js/shared/cookie-consent.js`
...
- Landing pages remain excluded from clean routing and continue using `index.php` and `admin/index.php`
...
- **File Organization**: Uploaded to `/uploads/id/` directory
